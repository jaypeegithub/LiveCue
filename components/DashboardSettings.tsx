...
export default function DashboardSettings() {
  ...
  // Recovery email (from user_metadata)
  ...
  useEffect(() => {
    getCurrentUser().then((u) => {
      ...
      const meta = u.user_metadata as { recovery_email?: string } | undefined;
      setRecoveryEmail(meta?.recovery_email ?? "");
    });
  ...
  async function handleSaveRecoveryEmail(e: React.FormEvent) {
    ...
    try {
      const { error } = await supabase.auth.updateUser({
        data: { recovery_email: value || null },
      });
  ...
  async function handleCancelWatch(watchId: string) {
    ...
    if (error) {
      console.error("[DashboardSettings] cancel watch", error);
      return;
    }
  ...
  return (
    <div className="livecue-main" style={{ maxWidth: 560 }}>
      {/* Account Settings */}
      <section style={{ marginBottom: "2.5rem" }}>
        <h2 className="livecue-section-title">Account Settings</h2>
            ...
            {passwordMessage && (
              <p
                style={{
                  marginBottom: "1rem",
                  fontSize: "0.9rem",
                  color: passwordMessage.type === "error" ? "#f87171" : "#86efac",
                }}
              >
            ...
            {recoveryMessage && (
              <p
                style={{
                  marginBottom: "1rem",
                  fontSize: "0.9rem",
                  color: recoveryMessage.type === "error" ? "#f87171" : "#86efac",
                }}
              >
        ...
        <div className="livecue-card">
          ...
          {!showDeleteConfirm ? (
            ...
            <div>
              ...
              {deleteError && (
                <p style={{ marginBottom: "1rem", fontSize: "0.9rem", color: "#f87171" }}>{deleteError}</p>
              )}
      ...
      <section>
        ...
        {watchesLoading ? (
          ...
          <p className="livecue-desc">You haven’t added any fights to watch yet. Use the form above to get alerted.</p>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            {watches.map((w) => {
              ...
              return (
                <div key={w.id} className="livecue-card" style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                  <div>
                  ...
                  {cancelConfirmId === w.id ? (
                    <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                      ...
                      <button
                        type="button"
                        className="livecue-btn"
                        style={{ padding: "0.4rem 0.75rem", fontSize: "0.85rem", maxWidth: "none" }}
                    ...
                    <button
                      ...
                      onClick={() => setCancelConfirmId(w.id)}
                    >
                      Cancel Watch
                    </button>
                  )}
